...
function PatternHeader({
  ...
  return (
    <header className="max-w-4xl mx-auto px-6 md:px-12 pt-16 pb-6">
      ...
      <h2 className="text-3xl md:text-4xl font-bold text-olive mb-3">{title}</h2>
      <p className="text-olive/95 leading-relaxed mb-3">{pitch}</p>
      <p className="text-sm text-olive/85 leading-relaxed">
        <strong className="text-olive">Best for:</strong> {best}
      </p>
      <p className="text-sm text-olive/85 leading-relaxed mt-1">
        <strong className="text-olive">Worst for:</strong> {worst}
      </p>
    </header>
  );
}
    ...
    <div className="relative aspect-[4/3] lg:aspect-auto lg:col-span-6 lg:min-h-[460px]">
      ...
      {route.successRate && (
        <div className="absolute top-4 right-4 bg-paper/95 backdrop-blur rounded-lg px-3 py-2 shadow-sm">
          ...
          <p className="text-gold-deep font-bold text-base leading-tight">{route.successRate}</p>
        </div>
      )}
    </div>
  );
  ...
  const content = (
    <div className="lg:col-span-6 p-6 md:p-8 flex flex-col">
      <div className="flex items-baseline gap-2 flex-wrap">
        <h3 className="text-2xl md:text-3xl font-bold text-olive leading-tight">{route.name}</h3>
        <span className="text-olive/75 text-base font-medium">
      ...
      <dl className="grid grid-cols-3 gap-4 mt-4 border-y border-taupe/30 py-3">
        <div>
          <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">From</dt>
          <dd className="text-olive font-bold text-base mt-1">{minPrice(route.durations)}</dd>
        </div>
        <div>
          <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Difficulty</dt>
          <dd className="text-olive font-medium text-sm mt-1">{route.difficulty}</dd>
        </div>
        <div>
          <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Scenery</dt>
          <dd className="text-olive text-sm mt-1">
            {"★".repeat(route.scenery)}
            <span className="text-olive/30">{"★".repeat(5 - route.scenery)}</span>
          </dd>
        </div>
      </dl>
      <p className="text-olive/95 text-sm leading-relaxed mt-4 line-clamp-3">
        {route.description}
      </p>
  ...
  return (
    <article className="bg-parchment rounded-2xl border border-taupe/20 overflow-hidden">
      <div className="relative aspect-[16/7]">
        ...
        />
        {route.tag && (
          <span className="absolute top-3 left-3 bg-paper/95 backdrop-blur text-olive text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-md">
            {route.tag}
          </span>
        )}
        {route.successRate && (
          <div className="absolute top-3 right-3 bg-paper/95 backdrop-blur rounded-md px-2.5 py-1.5 shadow-sm">
            <p className="text-olive/75 text-[9px] uppercase tracking-wider font-semibold leading-none">
              Success
            </p>
            <p className="text-gold-deep font-bold text-sm">{route.successRate}</p>
          </div>
        )}
      </div>
      <div className="px-6 py-5 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
        ...
        <div className="md:col-span-4">
          <div className="flex items-baseline gap-2 flex-wrap">
            <h3 className="text-2xl font-bold text-olive leading-tight">{route.name}</h3>
            <span className="text-olive/75 text-sm font-medium">
              {formatDaysRange(route.durations)}
            </span>
          </div>
          <p className="text-gold-deep italic text-sm">{route.nickname}</p>
        </div>
        {/* Stats */}
        <dl className="md:col-span-5 grid grid-cols-3 gap-3">
          <div>
            <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">From</dt>
            <dd className="text-olive font-bold text-sm mt-0.5">{minPrice(route.durations)}</dd>
          </div>
          <div>
            <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Difficulty</dt>
            <dd className="text-olive font-medium text-xs mt-0.5">{route.difficulty}</dd>
          </div>
          <div>
            <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Scenery</dt>
            <dd className="text-olive text-xs mt-0.5">
              {"★".repeat(route.scenery)}
              <span className="text-olive/30">{"★".repeat(5 - route.scenery)}</span>
            </dd>
          </div>
    ...
    <article className="bg-parchment rounded-2xl border border-taupe/20 overflow-hidden">
      <div className="relative aspect-[16/8]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
          {route.tag && (
            <span className="inline-block bg-paper/95 backdrop-blur text-olive text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-md mb-2">
              {route.tag}
            </span>
          )}
          <div className="flex items-baseline gap-2 flex-wrap">
            ...
            <span className="text-paper/90 text-base font-medium">
              {formatDaysRange(route.durations)}
            </span>
          </div>
          <p className="text-gold italic text-base">{route.nickname}</p>
        </div>
      </div>
      ...
      <div className="px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
        <dl className="flex gap-6">
          ...
          <div>
            <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Difficulty</dt>
            <dd className="text-olive font-medium text-sm mt-0.5">{route.difficulty}</dd>
          </div>
          <div>
            <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Scenery</dt>
            <dd className="text-olive text-sm mt-0.5">
              {"★".repeat(route.scenery)}
              <span className="text-olive/30">{"★".repeat(5 - route.scenery)}</span>
            </dd>
          </div>
  ...
  return (
    <section className="bg-paper border-t border-taupe">
      <PatternHeader
        ...
        pitch="Photo is the whole card (16:8 aspect). Title + nickname overlay on the photo bottom-left (paper text, olive gradient for legibility). Stats and CTA in a tight 50px strip below the photo. Most photo-forward of the four — feels cinematic, more 'destination ad' than 'product card.'"
...
function ListRow({ route }: { route: KilimanjaroRoute }) {
  return (
    <article className="bg-parchment rounded-2xl border border-taupe/20 overflow-hidden grid grid-cols-1 sm:grid-cols-[280px_1fr] hover:border-gold/40 transition-colors">
      <div className="relative aspect-[4/3] sm:aspect-auto sm:h-full sm:min-h-[180px]">
        ...
        />
        {route.tag && (
          <span className="absolute top-2 left-2 bg-paper/95 backdrop-blur text-olive text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-md">
            {route.tag}
          </span>
        )}
      </div>
      <div className="p-5 flex flex-col gap-2">
        <div className="flex items-baseline gap-2 flex-wrap">
          <h3 className="text-xl font-bold text-olive leading-tight">{route.name}</h3>
          <span className="text-gold-deep italic text-sm">{route.nickname}</span>
          <span className="text-olive/75 text-sm font-medium ml-auto">
            {formatDaysRange(route.durations)}
          </span>
        </div>
        <p className="text-olive/85 text-sm leading-snug line-clamp-2">{route.description}</p>
        <div className="flex items-center justify-between gap-3 mt-auto pt-1 flex-wrap">
          <dl className="flex gap-5 text-sm">
            <div className="flex items-baseline gap-1.5">
              <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">From</dt>
              <dd className="text-olive font-bold">{minPrice(route.durations)}</dd>
            </div>
            <div className="flex items-baseline gap-1.5">
              <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Diff</dt>
              <dd className="text-olive">{route.difficulty.split(" ")[0]}</dd>
            </div>
            {route.successRate && (
              <div className="flex items-baseline gap-1.5">
                <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Success</dt>
                <dd className="text-gold-deep font-bold">{route.successRate}</dd>
              </div>
            )}
            <div className="flex items-baseline gap-1.5">
              <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Scenery</dt>
              <dd className="text-olive text-xs">
                {"★".repeat(route.scenery)}
                <span className="text-olive/30">{"★".repeat(5 - route.scenery)}</span>
              </dd>
            </div>
          </dl>
          <span className="text-gold-deep font-semibold text-sm">View details →</span>
        </div>
      </div>
    </article>
  );
        ...
        <article className="bg-parchment rounded-2xl border border-taupe/20 overflow-hidden grid grid-cols-1 lg:grid-cols-12">
          ...
          <div className="lg:col-span-6 p-6 md:p-8 lg:p-10 flex flex-col">
            <div className="flex items-baseline gap-3 flex-wrap">
              <h3 className="text-3xl md:text-4xl font-bold text-olive leading-tight">{ROUTES[0].name}</h3>
              <span className="text-olive/75 text-lg font-medium">{formatDaysRange(ROUTES[0].durations)}</span>
            </div>
            <p className="text-gold-deep italic mt-1">{ROUTES[0].nickname}</p>
            <dl className="grid grid-cols-3 gap-4 mt-5 border-y border-taupe/30 py-4">
              <div>
                <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">From</dt>
                <dd className="text-olive font-bold text-base mt-1">{minPrice(ROUTES[0].durations)}</dd>
              </div>
              <div>
                <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Difficulty</dt>
                <dd className="text-olive font-medium text-sm mt-1">{ROUTES[0].difficulty}</dd>
              </div>
              <div>
                <dt className="text-olive/65 text-[10px] uppercase tracking-wider font-semibold">Scenery</dt>
                <dd className="text-olive text-sm mt-1">
                  {"★".repeat(ROUTES[0].scenery)}
                  <span className="text-olive/30">{"★".repeat(5 - ROUTES[0].scenery)}</span>
                </dd>
              </div>
            </dl>
            <p className="text-olive/95 text-sm md:text-base leading-relaxed mt-5">{ROUTES[0].description}</p>
            {ROUTES[0].highlights && (
              <ul className="mt-4 space-y-1.5">
                {ROUTES[0].highlights.slice(0, 3).map((h) => (
                  <li key={h} className="flex items-start gap-2 text-olive/95 text-sm">
                    <svg className="w-4 h-4 text-gold-deep shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function DesignPreviewPage() {
  return (
    <div className="bg-paper text-olive">
      <div className="max-w-3xl mx-auto px-6 py-12">
        <p className="text-xs uppercase tracking-wider text-gold-deep font-semibold mb-2">
        ...
        <p className="text-olive/95 leading-relaxed mt-3 text-sm">
        ...
        <p className="text-olive/85 leading-relaxed mt-3 text-sm">
      ...
      <div className="max-w-3xl mx-auto px-6 py-16 text-center">
        <p className="text-olive/85 text-sm">
